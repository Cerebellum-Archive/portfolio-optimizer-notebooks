...
class TestFactorModelOptimizer(TestPortfolioOptimizerBase):
    ...
    def test_custom_factor_inputs(self):
        ...
        # Fit a reference optimizer to generate baseline B, D, f, F
        ref_opt = FactorModelOptimizer()
        ref_opt.fit(self.ds_train)
        self.assertIsNotNone(ref_opt.B_)  # sanity check
        ...
        opt.fit(self.ds_train)

        # Optimizer must respect provided f and F
        np.testing.assert_array_almost_equal(opt.f_.values, f_in.values, decimal=6)
        np.testing.assert_array_almost_equal(opt.F_.values, F_in.values, decimal=6)

        # mu_ and sigma_ should be derived from custom f/F with B and D
        expected_mu = opt.B_.values @ f_in.values.flatten()
        expected_sigma = (
            opt.B_.values @ F_in.values @ opt.B_.values.T + opt.D_.values
        )
        np.testing.assert_array_almost_equal(opt.mu_.values, expected_mu, decimal=6)
        np.testing.assert_array_almost_equal(opt.sigma_.values, expected_sigma, decimal=6)

        # Internal factor computation should have been bypassed
        self.assertIsNone(opt.mu_residual_)
